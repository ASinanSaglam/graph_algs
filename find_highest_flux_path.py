...
def calculate_least_cost_path(input_graph):
   ...
   #This dictionary will hold every graph between every starting node and every ending node as values.  Keys are tuples of t(starting_node_name, ending_node_name).
   cost_and_path_list = []
        ...
        #We now must iterate over each ending node; that is, each node with a state of B
         for ending_node_name in input_graph.nodes():
		  ...
		  for adjacent_node_name in current_graph[current_node_name]:
		      ...
		      if current_graph.edge[current_node_name][adjacent_node_name]['k'] * current_graph.node[current_node_name]['cost'] > current_graph[adjacent_node_name]['cost']:
                         current_graph[adjacent_node_name]['cost'] = current_graph.edge[current_node_name][adjacent_node_name]['k'] * current_graph.node[current_node_name]['cost'] 
